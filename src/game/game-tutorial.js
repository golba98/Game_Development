...
const TUTORIAL_COIN_POSITIONS     = [{x:15,y:5},{x:16,y:4},{x:14,y:6}]; // collectible coin tiles
...
function loadTutorialMap() {
  ...
  try {
    ...
    const setTile = (x, y, tileType) => { mapStates[y * W + x] = tileType; };
    ...
    for (let y = TUTORIAL_WALL_Y; y < H; y++) setTile(TUTORIAL_WALL_X, y, TILE_TYPES.FOREST);
    ...
    initialSpawnPosition = { x: TUTORIAL_PLAYER_SPAWN_X, y: TUTORIAL_PLAYER_SPAWN_Y };
...
function _setTutorialMsg(msg, durationMs) {
  if (tutorialMessageTimer > TUTORIAL_MSG_HOLD_MS && tutorialMessage !== msg) return;
  tutorialMessage = msg;
...
function handleTutorialLogic() {
  ...
  if (tutorialStep === TUTORIAL_STEP_MOVE) {
    if (pX !== TUTORIAL_PLAYER_SPAWN_X || pY !== TUTORIAL_PLAYER_SPAWN_Y) tutorialMoved = true;
    if (pY < TUTORIAL_WALL_Y) _advanceTutorial(TUTORIAL_STEP_SPRINT, t('tut_made_it'));
  }
  // Step 1: SPRINT — brief sprint lesson in top zone
    ...
    if (tutorialSprintDetected || tutorialStepTimer > TUTORIAL_SPRINT_TIMEOUT_MS) {
      _advanceTutorial(TUTORIAL_STEP_COMBAT, tutorialSprintDetected ? t('tut_fast2') : '');
    }
    if (pY >= TUTORIAL_WALL_Y) _setTutorialMsg(t('tut_come_back_sprint', _sprintKeyLabel()), TUTORIAL_MSG_DEFAULT_MS);
  }
  // Step 2: COMBAT — defeat the beetle dummy
    ...
    if (enemies.length === 0) _advanceTutorial(TUTORIAL_STEP_COINS, t('tut_pest_eliminated'));
    if (enemies.length > 0 && pX > TUTORIAL_WALL_X && pY < TUTORIAL_WALL_Y) _setTutorialMsg(t('tut_defeat_first'), TUTORIAL_MSG_DEFAULT_MS);
    if (pY >= TUTORIAL_WALL_Y) _setTutorialMsg(t('tut_pest_north'), TUTORIAL_MSG_DEFAULT_MS);
  }
  // Step 3: COLLECT COINS
    ...
    if (pX < TUTORIAL_GAP1_X && pY < TUTORIAL_WALL_Y) _setTutorialMsg(t('tut_coins_east'), TUTORIAL_MSG_DEFAULT_MS);
    if (pY >= TUTORIAL_WALL_Y) _setTutorialMsg(t('tut_collect_north'), TUTORIAL_MSG_DEFAULT_MS);
  }
  // Step 4: ENTER PORTAL
  else if (tutorialStep === TUTORIAL_STEP_PORTAL) {
    if (pY >= TUTORIAL_WALL_Y) _setTutorialMsg(t('tut_portal_north'), TUTORIAL_MSG_DEFAULT_MS);
    else if (pX < TUTORIAL_GAP1_X + 3 && pY < TUTORIAL_WALL_Y) _setTutorialMsg(t('tut_portal_east'), TUTORIAL_MSG_DEFAULT_MS);
  }
}
...
function drawTutorial() {
  ...
  if (tutorialStep === TUTORIAL_STEP_MOVE && !warningActive) {
    ...
    else { drawPlayerTooltip(t('tut_head_north'), { size: 18 }); drawArrow(TUTORIAL_GAP1_X, TUTORIAL_WALL_Y, t('tut_gap')); }
  }
  ...
  if (tutorialMessage && tutorialMessageTimer > 0) {
    ...
    const my = (virtualH || (height / gameScale)) * 0.35 + Math.sin(now * 0.003) * 3;
