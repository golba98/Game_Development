function preload() {
  ...
  try {
    trackLoadImage(
      "button_bg",
      "assets/3-GUI/Button_BG.png",
      (img) => {
        BUTTON_BG = img;
        verboseLog(
          "[game] loaded BUTTON_BG",
          img && img.width,
          "x",
          img && img.height,
        );
      ...
      (err) => {
        console.warn("[game] failed to load BUTTON_BG", err);
        BUTTON_BG = null;
      },
    );
  } catch (e) {}
  ...
  try {
    trackLoadImage(
      "tile_1",
      "assets/1-Background/2-Game/1-Forest/tile_1.png",
      (img) => {
        TILE_IMAGES["tile_1"] = img;
      },
      (err) => {
        TILE_IMAGES["tile_1"] = null;
      },
    ...
    trackLoadImage(
      "tree_1",
      "assets/1-Background/2-Game/1-Forest/tree_1.png",
      (img) => {
        TILE_IMAGES["tree_1"] = img;
      },
      (err) => {
        TILE_IMAGES["tree_1"] = null;
      },
    ...
    trackLoadImage(
      "water_1",
      "assets/1-Background/2-Game/1-Forest/water_1.png",
      (img) => {
        TILE_IMAGES["water_1"] = img;
        try {
          TILE_IMAGES[TILE_TYPES.RIVER] = img;
        } catch (e) {}
      },
      (err) => {
        TILE_IMAGES["water_1"] = null;
    ...
    trackLoadImage(
      "bridge_1",
      "assets/1-Background/2-Game/1-Forest/bridge_1.png",
      (img) => {
        TILE_IMAGES["bridge_1"] = img;
        try {
          TILE_IMAGES[TILE_TYPES.RAMP] = img;
          TILE_IMAGES[TILE_TYPES.LOG] = img;
        } catch (e) {}
      },
      (err) => {
        TILE_IMAGES["bridge_1"] = null;
  ...
  if (TREE_OVERLAY_PATH) {
    try {
      trackLoadImage(
        "treeoverlay:" + TREE_OVERLAY_PATH,
        TREE_OVERLAY_PATH,
        (img) => {
          TREE_OVERLAY_IMG = img;
          verboseLog(
            "[game] loaded tree overlay",
            TREE_OVERLAY_PATH,
            img.width,
            "x",
            img.height,
        ...
        (err) => {
          console.warn(
            "[game] failed to load tree overlay",
            TREE_OVERLAY_PATH,
            err,
          );
  ...
  Object.entries(DECOR_ASSET_PATHS).forEach(([name, path]) => {
    try {
      trackLoadImage(
        ...
        path,
        (img) => {
          DECOR_ASSET_IMAGES[name] = img;
        },
        (err) => {
          console.warn("[game] failed to load decor asset", name, err);
        },
      );
    } catch (e) {
  ...
  try {
    logAssetTrackerStatus("after preload queue");
  } catch (e) {}
  ...
  try {
    trackLoadImage(
      "mantis_move",
      "assets/2-Characters/5-Enemies/MantisMove.png",
      (img) => {
        mantisMoveSprite = img;
        verboseLog("[game] loaded MantisMove.png");
      },
      (err) => {
        console.warn("[game] failed to load MantisMove.png", err);
      },
    );

    trackLoadImage(
      "mantis_attack",
      "assets/2-Characters/5-Enemies/MantisAttack.png",
      (img) => {
        mantisAttackSprite = img;
        verboseLog("[game] loaded MantisAttack.png");
      },
      (err) => {
        console.warn("[game] failed to load MantisAttack.png", err);
      },
    );

    trackLoadImage(
      "beetle_move",
      "assets/2-Characters/5-Enemies/BeetleMove.png",
      (img) => {
        beetleMoveSprite = img;
        verboseLog("[game] loaded BeetleMove.png");
      },
      (err) => {
        console.warn("[game] failed to load BeetleMove.png", err);
      },
    );

    trackLoadImage(
      "beetle_attack",
      "assets/2-Characters/5-Enemies/BeetleAttack.png",
      (img) => {
        beetleAttackSprite = img;
        verboseLog("[game] loaded BeetleAttack.png");
      },
      (err) => {
        console.warn("[game] failed to load BeetleAttack.png", err);
      },
    );

    trackLoadImage(
      "maggot_walk",
      "assets/2-Characters/5-Enemies/MaggotWalk.png",
      (img) => {
        maggotWalkSprite = img;
        verboseLog("[game] loaded MaggotWalk.png");
      },
      (err) => {
        console.warn("[game] failed to load MaggotWalk.png", err);
      },
    );

    trackLoadImage(
      "maggot_spit",
      "assets/2-Characters/5-Enemies/MaggotSpit.png",
      (img) => {
        maggotSpitSprite = img;
        verboseLog("[game] loaded MaggotSpit.png");
      },
      (err) => {
        console.warn("[game] failed to load MaggotSpit.png", err);
      },
    );

    trackLoadImage(
      "acid_blob",
      "assets/2-Characters/5-Enemies/AcidBlob.png",
      (img) => {
        acidBlobSprite = img;
        verboseLog("[game] loaded AcidBlob.png");
      },
      (err) => {
        console.warn("[game] failed to load AcidBlob.png", err);
      },
    );

    trackLoadImage(
      "heart",
      "assets/3-GUI/Heart.png",
      (img) => {
        heartImage = img;
        verboseLog("[game] loaded Heart.png");
      },
      (err) => {
        console.warn("[game] failed to load Heart.png", err);
      },
    );

    trackLoadImage(
      "coin_anim",
      "assets/6-Icons/coin_4_frames.png",
      (img) => {
        coinAnimSprite = img;
        verboseLog("[game] loaded coin_4_frames.png");
      },
      (err) => {
        console.warn("[game] failed to load coin_4_frames.png", err);
      },
    );

    trackLoadImage(
      "acid_splat",
      "assets/2-Characters/5-Enemies/AcidSplat.png",
      (img) => {
        acidSplatSprite = img;
        verboseLog("[game] loaded AcidSplat.png");
      },
      (err) => {
        console.warn("[game] failed to load AcidSplat.png", err);
      },
    );

    trackLoadImage(
      "eggsplosion",
      "assets/2-Characters/5-Enemies/Eggsplosion.png",
      (img) => {
        eggsplosionSprite = img;
        verboseLog("[game] loaded Eggsplosion.png");
      },
      (err) => {
        console.warn("[game] failed to load Eggsplosion.png", err);
      },
    );

    trackLoadImage(
      "ghost_move",
      "assets/2-Characters/5-Enemies/GhostMove.png",
      (img) => {
        try {
          const cols = 3,
            rows = 4,
          ...
          const targetW = Math.round(
            targetH * (img.width / cols / (img.height / rows)),
          );
      ...
      (err) => {
        console.warn("[game] failed to load GhostMove.png", err);
      },
    );

    trackLoadImage(
      "ghost_attack",
      "assets/2-Characters/5-Enemies/GhostAttack.png",
      (img) => {
        try {
          const cols = 3,
            rows = 4,
          ...
          ghostAttackSprite = img;
          verboseLog(
            "[game] loaded GhostAttack.png (raw, downscale failed)",
            e,
          );
        }
      },
      (err) => {
        console.warn("[game] failed to load GhostAttack.png", err);
      },
    );

    trackLoadImage(
      "powerup_sprite",
      "assets/2-Characters/5-Enemies/EggCluster.png",
      (img) => {
        powerupSprite = img;
        verboseLog("[game] loaded EggCluster.png");
      },
      (err) => {
        console.warn("[game] failed to load EggCluster.png", err);
      },
    );

    trackLoadImage(
      "portal_active",
      "assets/5-Objects/2-Portal/portal_active_sheet.png",
      (img) => {
        portalActiveSheet = img;
      },
      (err) => {
        console.warn("[game] failed to load portal_active_sheet.png", err);
      },
    );

    trackLoadImage(
      "portal_inactive",
      "assets/5-Objects/2-Portal/portal_inactive_sheet.png",
      (img) => {
        portalInactiveSheet = img;
      },
      (err) => {
        console.warn("[game] failed to load portal_inactive_sheet.png", err);
      },
    );
  } catch (e) {}
}
...
function createPotionGraphics(liquidColor) {
  ...
  pg.noStroke();
  pg.rect(13, 3, 6, 4, 1);
  pg.fill(160, 82, 45); // Cork highlight
  pg.rect(13, 3, 6, 1, 1);
  ...
  // Bottle body background (Glass Back)
  pg.fill(150, 160, 170, 90);
  pg.noStroke();
  pg.circle(16, 20, 18);
  pg.rect(13, 9, 6, 6);

  // Liquid
  // Extract RGB components if it's an array
  let r = liquidColor[0];
  let g = liquidColor[1];
  let b = liquidColor[2];
  ...
  // Liquid glow/fill
  pg.fill(r, g, b, 240);
  pg.arc(16, 20, 16, 16, 0, Math.PI); // Bottom half of sphere
  pg.rect(10, 18, 12, 4); // Middle of liquid

  // Liquid top surface (ellipse for depth)
  pg.fill(r + 30, g + 30, b + 30, 255);
  pg.ellipse(16, 18, 12, 4);
  ...
  // Glass specular highlight (left curve)
  pg.noFill();
  pg.stroke(255, 255, 255, 200);
  pg.strokeWeight(1.5);
  pg.strokeCap(ROUND);
  pg.arc(16, 20, 14, 14, Math.PI + 0.3, Math.PI + 1.2);

  // Neck highlight
  pg.strokeWeight(1);
  pg.line(14, 10, 14, 13);

  // Bottle outline (Glass Front/Rim)
  pg.noFill();
  pg.strokeCap(SQUARE);
  pg.stroke(240, 240, 255, 160);
  pg.strokeWeight(1.5);
  pg.circle(16, 20, 18);
      ...
      try {
        while (this._callbacks && this._callbacks.length) {
          ...
          try {
            ...
            console.warn("[AssetTracker] onReady callback threw", e);
          }
        }
      } catch (e) {}
    }
  ...
  onReady(cb) {
    ...
    if (this.loaded >= this.expected) {
      try {
        ...
        console.warn("[AssetTracker] onReady immediate callback threw", e);
      }
      return;
    }
  ...
  try {
    loadImage(
      ...
      (img) => {
        ...
        try {
          AssetTracker.markLoaded(key || path);
        } catch (e) {}
      },
      (err) => {
        try {
...
function trackLoadSound(key, path, successCb, errorCb) {
  ...
  try {
    loadSound(
      ...
      (snd) => {
        try {
          if (typeof successCb === "function") successCb(snd);
        } catch (e) {}
        try {
          AssetTracker.markLoaded(key || path);
        } catch (e) {}
...
function scheduleDeferredCharacterAssets() {
    ...
    Object.entries(paths).forEach(([dir, path]) => {
      ...
      try {
        trackLoadImage(
          ...
          (err) => {
            console.warn("[game] failed to load", label, dir, path, err);
            targetMap[dir] = null;
          },
        );
      } catch (e) {
  ...
  const loadCloudTextures = () => {
    ...
    const targetCount =
      typeof CLOUD_IMAGE_COUNT === "number" ? CLOUD_IMAGE_COUNT : 4;
    for (let i = 1; i <= targetCount; i++) {
      ...
      try {
        trackLoadImage(
          ...
          (err) => {
            console.warn("[game] failed to load cloud", i, err);
          },
        );
      } catch (e) {
  ...
  const runDeferredLoads = () => {
    try {
      logAssetTrackerStatus("before deferred character assets");
    } catch (e) {}
    ...
    loadCloudTextures();
    try {
      logAssetTrackerStatus("after deferred character assets");
    } catch (e) {}
  };
  ...
  try {
    logAssetTrackerStatus("deferred character assets scheduled");
  } catch (e) {}
}
...
function releaseGameAssets() {
  ...
  try {
    if (gameMusic && typeof gameMusic.stop === "function") gameMusic.stop();
  } catch (e) {}
  gameMusic = null;
  clickSFX = null;
